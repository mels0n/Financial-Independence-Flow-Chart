export const glossary: Record<string, string> = {
    "HSA": "Health Savings Account. A triple-tax-advantaged account available to those with a High Deductible Health Plan (HDHP). Contributions are tax-deductible, growth is tax-free, and withdrawals for medical expenses are tax-free.",
    "Roth": "A type of account where you pay taxes on money NOW, but it grows tax-free and withdrawals in retirement are tax-free.",
    "Traditional": "A type of account where you deduct contributions from your taxes NOW (pay less tax today), but pay taxes when you withdraw the money in retirement.",
    "401k": "An employer-sponsored retirement savings plan. Allows you to save a portion of your paycheck before taxes (Traditional) or after taxes (Roth).",
    "Deductible": "The amount you pay for covered health care services before your insurance plan starts to pay.",
    "APR": "Annual Percentage Rate. The yearly interest you are charged on borrowed money.",
    "MAGI": "Modified Adjusted Gross Income. Your household income with certain deductions added back. Used to determine eligibility for Roth IRA contributions.",
    "FICA": "Federal Insurance Contributions Act. A U.S. federal payroll tax covering two taxes: Social Security and Medicare.",
    "VTSAX": "Vanguard Total Stock Market Index Fund. A very popular low-cost mutual fund that invests in the entire US stock market.",
    "Backdoor Roth": "A strategy for high earners to contribute to a Roth IRA correctly. It involves contributing to a Traditional IRA and then converting it to a Roth IRA.",
};
